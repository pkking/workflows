...
def render(runs: list[dict[str, Any]], generated: dt.datetime, api_calls: int) -> str:
    dominant = [(run, max(run["findings"], key=lambda item: item["duration"], default=None)) for run in runs]
    queue_bound = sum(item is not None and item["kind"] == "Runner queue" for _, item in dominant)
    execution_bound = sum(item is not None and item["kind"] == "Job execution" for _, item in dominant)
    incomplete = sum(run["status"] != "completed" for run in runs)
    data_gaps = sum(step["timestamp_error"] for run in runs for job in run["jobs"] for step in job["steps"])
    overview_rows = []
    for run, cause in sorted(dominant, key=lambda pair: pair[0]["wall"] or -1, reverse=True)[:10]:
        cause_text = "—" if cause is None else f"{cause['kind']}: {cause['subject']}"
        overview_rows.append(f'''<tr><td><a href="#run-{run['run_id']}-{run['attempt']}">{run['run_id']} / {run['attempt']}</a></td><td>{minutes(run['wall'])}</td><td>{html.escape(cause_text)}</td><td>{minutes(cause['duration']) if cause else '—'}</td><td><a href="{html.escape(run['url'], quote=True)}">GitHub ↗</a></td></tr>''')
    overview = f'''<section class="overview"><h3>Fleet summary</h3><div class="scoreboard"><article><b>{len(runs)}</b><span>Attempts analyzed</span></article><article><b>{execution_bound}</b><span>Execution dominated</span></article><article><b>{queue_bound}</b><span>Queue dominated</span></article><article><b>{incomplete}</b><span>Still incomplete</span></article><article><b>{data_gaps}</b><span>Invalid Step timestamps</span></article></div><p class="method">Dominant timing bottleneck = the largest directly measured queue or execution interval in each attempt. Proxy findings remain supporting evidence, not semantic root cause.</p><h3>Longest attempts</h3><div class="table-wrap"><table><thead><tr><th>Run / attempt</th><th>Wall clock</th><th>Dominant timing bottleneck</th><th>Duration</th><th>Source</th></tr></thead><tbody>{''.join(overview_rows)}</tbody></table></div></section>'''
    sections = []
    for run in runs:
...
<main><header class="hero"><div><div class="eyebrow">GITHUB ACTIONS · TIMING FORENSICS</div><h1>Workflow<br><em>flight recorder</em></h1></div><p>Queue, execution, and every reported Step aligned to real wall clock. Timing evidence localizes where the minutes went; logs are required to prove semantic causes.</p></header>{overview}{''.join(sections)}
