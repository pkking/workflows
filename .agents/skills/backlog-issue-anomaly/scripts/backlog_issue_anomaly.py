...
def check_openpyxl():
    try:
        ...
        die(
            "openpyxl not installed. Install with:  pip install openpyxl  (or:  pip3 install openpyxl)"
        )
    return openpyxl
...
def fetch_issues(token, owner, name, states):
    ...
    while True:
        data = gql(
            token,
            QUERY,
            {"owner": owner, "name": name, "cursor": cursor, "states": states},
        )
...
def summarize_issue(it):
    labels = {lb["name"] for lb in _nodes(it.get("labels"))}
    ...
    for ev in _nodes((it.get("timelineItems") or {}).get("nodes")):
        if ev["__typename"] == "LabeledEvent":
            if (ev.get("label") or {}).get(
                "name"
            ) == "accepted" and accept_time is None:
...
def classify(s, days):
    ...
    stale_open = [
        p
        for p in s["open_prs"]
        if days_since(p["updatedAt"]) is not None and days_since(p["updatedAt"]) > days
    ]
...
def write_excel(flagged, user_map, path, days, scanned_open, scanned_closed, repo):
    ...
    rows_sorted = sorted(
        by_user.items(), key=lambda kv: sum(kv[1]["counts"].values()), reverse=True
    )
    ...
    meta = [
        ...
        [
            "5-开发停滞",
            "open & accepted & 有 assignee+milestone & 无 PR & accept 超过 N 天",
        ],
        ...
        [
            "注",
            "backlog 跟踪的交付常在其他仓(om-datacenter/xihe 等);未在 issue 内回链的跨仓 PR 检测不到,故 5/7 为上界。",
        ],
    ]
    for r in meta:
        ws0.append(r)
...
def main():
    ap = argparse.ArgumentParser(
        description="Scan a GitHub backlog repo for stuck/anomalous issues; output Excel."
    )
    ap.add_argument(
        "--repo",
        default="opensourceways/backlog",
        help="owner/name (default opensourceways/backlog)",
    )
    ap.add_argument(
        "--days", type=int, default=7, help="staleness window in days (default 7)"
    )
    ap.add_argument(
        "--closed",
        action="store_true",
        help="also scan closed issues (enables scenarios 7 & 8)",
    )
    ap.add_argument(
        "--output",
        "-o",
        default="backlog_anomaly_report.xlsx",
