...
The backlog repo tracks work that is **delivered in other repos** (om-datacenter, xihe, community-robots, …). Cross-referenced PRs from those repos _do_ appear in the backlog issue timeline **if and only if** the PR referenced the issue. PRs that landed without referencing the issue are invisible to issue-side detection. Consequences:

- **S5 (开发停滞) and S7 (异常关闭) are upper bounds.** An issue flagged here _might_ actually have been delivered by an unreferenced cross-repo PR. Treat as "needs review", not "definitely broken".
