...
- **Cross-repo caveat (read this):** backlog tracks work delivered in _other_ repos (om-datacenter, xihe, community-robots, …). Cross-referenced PRs from those repos appear in the issue timeline **only if the PR referenced the issue**. PRs that landed without referencing it are invisible to issue-side detection, so **S5 (开发停滞) and S7 (异常关闭) are upper bounds** — an issue flagged here _might_ have been delivered by an unreferenced cross-repo PR. Treat as "needs review". For ground truth on one issue, run `gh pr list --repo <delivery-repo> --search "<issue#>"` in the likely delivery repo.
