...
* **Location**: the retired `workflow_runs_on_date.py` adapter (historical analysis)
...
* **ADR-001 (Turso DB based CI analysis)**:
  * *Alignment*: The primary module `ci_analyze.py` successfully implements SQL queries against the Turso database to bypass GitHub rate limits. Current REST-based, run-centric workflow duration analysis is implemented by the root-level skill.
